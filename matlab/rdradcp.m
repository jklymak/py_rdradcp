...
%         1/2/3/4 in beam coordinates, stbd/forward/vertical/error for ship
%         coords, etc. (this also applies to the order in which bottom-track
%         velocities appear).
%
%       - (new for 2024) By adding the 'SvEr' option below the backscatter
%         intensity can be converted (approximately) into a scattering
%         strength estimate.
%  
...
%                  data).
%  'sourceprog' : ['instrument' | 'WINRIVER' | 'WINRIVER2' 'VMDAS']
%                 the program tries to identify the data source by looking
%                 at the available data blocks in two ensembles, but if the
%                 the types of blocks vary from ensemble to ensemble then
%                 this might fail. Use this option to force the datatype
%                 (usually only if the program output suggests it). Use of
%                 this option SHOULD be extremely rare.
%  'debug'    :   [ 'no' (default) | 'yes' ] Outputs debugging info (Gives
%                 block IDs - useful to see if something undecoded has
%                 appeared for a new RDI instrument or software package)
%
% The following options are needed if you want the backscatter converted
% into a reasonably calibrated Scattering Volume estimate. If done
% correctly, tpyically you will get Sv estimates between about -100 and -40 
% db re 1m^2 (4*pi*m^3)^{−1}.  See code from line 502 onwards for more
% details.
%
%  'SvEr'    :   [ 0 (default)| Scalar | 4 numbers] The thermal noise RSSI 
%                 (intensity) for Sv calculation. Should be around 40, so 
%                 you can try that first  if you want, but a better number can be 
%                 obtained by looking at the stored intensity data at maximum range.
%                 To look at these, first set this to 0 and 'raw' intensities will
%                 be output by RDRADCP. Then examine the raw intensity data
%                 to get a better estimate, and finally set this parameter to
%                 the better value as you re-run RDRACP! A  new field will appear, 
%                 Sv_corr, so that ADCP.intens(:,n,:)+ADCP.Sv_corr should be 
%                 (approximately)  the Scattering Volume Strength.
%                 If different values are used for each beam, then use a 4
%                 element vector with the values for beams 1-4.
%                 These values are also available from RDI.
%  'SvKc'     :   [  Scalar | 4 numbers] The ADC conversion
%                 factor. If not specified we try to use an appropriate
%                 default for your instrument (0.45 usually). Otherwise set to
%                 scalars (1 for each beam) which you can get from RDI for a 
%                 particular instrument serial #.
%  'STP'      :   [ [30 10 0] | 3 element vector ] Salinity, temperature
%                 pressure for absorption correction if you are using the
%                 Sv_corr calculation.
%
% R. Pawlowicz (rich@eos.ubc.ca) - original 17/09/99
...
%   25/Aug/2023 - Added try/catch around some GPS time decoding, as we had
%                 WinriverII datafile in which no timestamps appeared in
%                 some sentences.
%   25/Oct/2023 - M. Otero; Added Pioneer DVL firmware 57.26 support
%   30/Nov/2023 - added option to 'force' WINRIVER type - it turns out that
%                 if ping rate is fast enough, not all pings might have a
%                 GPS block!
%   8/Sept/2024 - Added 'debug' option, also HDT decoding for WINRIVER2.
%  14/Sept/2024 - Added calculation of Correction factor to range-correct 
%                 intensity to scattering volume.
%  17/Feb/2025  - decoded current and voltage from ADC
%  19/Feb/2025  - added support for RiverPro.

global STANDARD DEBUGFLAG  
...
ftype=[];      % Take default for determining source program.
DEBUGFLAG=false;

% Parameters for the Scattering volume correction. 
Sv.STP=[30 10 0];
Sv.Er=0;
Sv.Kc=0.45;
...
while ~isempty(varargin)
 switch lower(varargin{1}(1:3))
     ...
     case 'sve'
         Sv.Er=varargin{2};
         Sv.Er=Sv.Er(:)'; % Make it a row
     case 'svk'
         Sv.Kc=varargin{2};
         Sv.Kc=Sv.Kc(:)'; % Make it a row
     otherwise
...
% Force the source program identifier.
if ~isempty(ftype)
  cfg.sourceprog=ftype;
end

fseek(fd,pos,'bof');              % Rewind

fprintf(' File comes from %s (firmware %5.2f), \n source of data from: %s (based on presence/absence of GPS data in ensembles 1/2)\n',cfg.name,cfg.prog_ver,cfg.sourceprog);
...
if (cfg.prog_ver<16.05 && cfg.prog_ver>5.999) || cfg.prog_ver<5.55 || (cfg.prog_ver>=23 && cfg.prog_ver<30) || cfg.prog_ver==57.26
...
switch cfg.sourceprog
  case {'WINRIVER','WINRIVER2'}
    ...
    adcp=struct('name','adcp','config',cfg,'mtime',zeros(1,n),'number',zeros(1,n),'pitch',zeros(1,n),...
                ...
                'voltage',zeros(1,n),'current',zeros(1,n),...
                'coords','',...
                ...
                'bt_range',zeros(nbot,n),'bt_vel',zeros(4,n),...
...
for k=1:n
  ...
  [ens,hdr2,cfg2]=rd_buffer(fd,num_av);

  if ~isstruct(ens) % If aborting...
    adcp.rerunparameter=(k-1)*num_av;
    fprintf('Only %d records found..suggest re-running RDRADCP using NENS as this value\n',(k-1)*num_av);
    fprintf('(If this message preceded by a POSSIBLE PROGRAM PROBLEM message, re-run using %d)\n',(k-1)*num_av-1);
    break;
  end
    
  % If a WINRIVER block shows up later make a note
  if strcmp(cfg.sourceprog,'instrument') && ~strcmp(cfg.sourceprog,cfg2.sourceprog)
      fprintf('New blocks appeared unexpectedly - rerun with ...''sourceprog'',''%s'' option\n',cfg2.sourceprog)
  end
 ...
 % fprintf('%f %f\n',ens.heading(1),adcp.heading(k));
  ...
  adcp.voltage(k)     =mean(ens.adc(2,:));
  adcp.current(k)     =mean(ens.adc(1,:));
 ...
 %%%   disp(['--' datestr(adcp.nav_mtime(k),'HH:MM:SS')]);
   ...
   case 'VMDAS'
     ...
     zz=ens.speedmadegd.*exp(1i*pi/180*(90-ens.direcmadegd));  % Don't average speed and angles
...
if any(Sv.Er~=0)
    % Range-dependent correction for spreading and absorption to 
    % get  scattering volume strength.
    %
    % Refs are:
    %
    % [1] K. L. Deines, "Backscatter estimation using Broadband acoustic Doppler current profilers," 
    % Proceedings of the IEEE Sixth Working Conference on Current Measurement (Cat. No.99CH36331), 
    % San Diego, CA, USA, 1999, pp. 249-253, doi: 10.1109/CCM.1999.755249.
    %
    % [2] J. W. Mullison, "Backscatter Estimation Using ADCPs - Twenty Years Later," 2019 IEEE/OES Twelfth 
    % Current, Waves and Turbulence Measurement (CWTM), San Diego, CA, USA, 2019, pp. 
    % 1-5, doi: 10.1109/CWTM43797.2019.8955274.
    %
    % [3] J. W. Mullison, "Backscatter Estimation Using ADCPs - Updated, Teledyne-RDI Application 
    % note FSA-031.


    % Noise level should be around 40, and you can often get this by looking
    % at the furthest bin, but for consistency I am going to take this as a
    % constant. THis is now set at the top of this function.
    %Er=mean(nanmedian(adcp.intens(end,:,end),3)); % RSSI Noise level (maybe furthest away?)
    %if abs(Er-(40))<20
    %    fprintf('Instrument noise level estimate of %f may be incorrect\n',Er);
    %end

    % Workhorse manual suggests 0.45 dB per count as a good approximation
    % This also appears to be the right number for BBADCPs.
    % (instrument-to-instrument range of 0.35-0.55 suggested by RDI [1])
    % RDI can provide a number for particular instruments.
    % For RiverRay/Pro Explorer and Pioneer systems Kc is calibrated to be
    % 0.60, and Er=50, and for Sentinel V systems kc=0.50 and Er=40. 
   
    
    % Constant "C" and Transmit power "PdBW"
    % Note that power varies with input Voltage, and for battery-operated
    % units can vary by 6dB over the life of the battery so these are "typical"
    % values extracted from Table 1 of [1] (bb-adcp), and from table 2 of [3].
    % Table 3 of [3] also provides a way to correct source power based on
    % measured voltages, which are in fact provided in the PD0 files but
    % I have't decoded them in this code. Also, note that if you switch
    % instruments to narrow-band (for longer range) then the 'C' constant
    % changes, this too I have ignored right now. Can be checked with
    % cfg.sysbandwith (right now I assume this is =0).

    % Actual frequencies (afreq) taken from FST-054.

    C_PdBW=-150; % Default if not in this table?
    switch cfg.name
        case 'bb-adcp'
            switch cfg.beam_freq
                case 75
                    C_PdBW= -163.3 - 23.8;
                    afreq=76.8;
                case 150
        ...
        case 'wh-adcp'
            switch cfg.beam_freq
                case 75
                    C_PdBW= -153.75 - 18.6;  % Quartermaster
                    afreq=76.8;
                case 300
                    C_PdBW= -140.87 - 17.5;  % Power supply  
                    afreq=307.2;
                case 600
                    C_PdBW= -139.09 - 12.5;  % Power supply
                    afreq=614.4;
                case 1200
                    C_PdBW= -129.44 - 8.3;   % Power supply
                    afreq=1228.8;
            end
        case 'os-adcp'
           switch cfg.beam_freq
         ...
         case 'v-adcp'
            switch cfg.beam_freq
                case 1000 % V20
                    C_PdBW= -135.49  - 9.0;  % Battery
                     afreq=983.0;
               case 500  % V50
                    C_PdBW= -139.18 - 10.8;  % Battery
                    afreq=491.5;
                case 300  % V100
                    C_PdBW= -144.74 - 14.0;  % Battery
                    afreq=307.2;
            end
            Sv.Kc=0.50;
        case 'river'
    ...
    % Note that I have the SNR here as  
    %  
    %adcp.intens=Kc*(adcp.intens-Er);
    %
    % from [1], but according to [2] this should be
    %
    adcp.intens=10*log10(10.^(Sv.Kc.*(max(adcp.intens-Sv.Er,0))/10)-1);
    %
    % which will be slightly different for signals near the noise level.
    % Note - in recent version of matlab, this will also do the right thing
    % if Kc and Er are rows with 4 elements.
    %
    % Apparently the intensity sample is in the last quarter of each depth cell
    % and not the center, so we want a slightly larger range in
    % calculations.
    slantRange=abs(cfg.ranges+cfg.cell_size/4)/cosd(cfg.beam_angle);
     
    % This is added to SNR output in intens to get Sv.
    adcp.Sv_corr = C_PdBW + 10*log10(273.15+median(adcp.temperature)) ...
                   + 20*log10(slantRange) ...
                   - 10*log10(cfg.xmit_pulse) ...
                   + 2*absorb(afreq,Sv.STP(1),Sv.STP(2),Sv.STP(3))*(slantRange/1e3);
...
%if strcmp(cfg.name,'river')  % They SAY 600 but that's only the central beam
   % cfg.beam_freq=1200;      % in fact its 1200...maybe?
%end
...
cfg.xmit_pulse     =fread(fd,1,'uint16')*.01;   % meters (Transmit pulse length)
if strcmp( cfg.name, 'pioneer-dvl' )
...
% SOURCE is 0 regular file: 'instrument'
...
cfg=NaN; % define this here as for really short blpcks at the end
         % cfg might not get defined, which leads to problems in the
         % calling program.

% If num_av<0 we are reading only first elements and initializing
if num_av<0
 SOURCE=0;
end 
...
if num_av<0 || isempty(ens)
 ...
 ens=struct('number',zeros(1,n),'rtc',zeros(7,n),'BIT',zeros(1,n),'ssp',zeros(1,n),'depth',zeros(1,n),'pitch',zeros(1,n),...
            ...
            'bt_range',zeros(4,n),'bt_vel',zeros(4,n),...
            'bt_corr',zeros(4,n),'bt_ampl',zeros(4,n),'bt_perc_good',zeros(4,n),...
            'smtime',zeros(1,n),'emtime',zeros(1,n),'slatitude',NaN(1,n),...
            'slongitude',NaN(1,n),'elatitude',NaN(1,n),'elongitude',NaN(1,n),...
...
% If GPS blocks don't always appear, this is needed to prevent previouis
% positions from being copied over.
[ens.smtime,ens.emtime,ens.slatitude,ens.slongitude,ens.elatitude,ens.elongitude]=deal(NaN(size(ens.smtime)));
...
%% fprintf('ID=%s SOURCE=%d  k=%d n=%d\n',id,SOURCE,k,n);%%pause;
  ...
  %%     fprintf('NEW ID=%s SOURCE=%d\n',id,SOURCE);
   ...
   if DEBUGFLAG, fprintf('Block id = %s\n',id); end
 ...
 % to decode the info in each block and return this to the user.
  ...
  %  fprintf('%d %d ',ens.adc(1:2,k)); 
      if strcmp(cfg.name,'wh-adcp')
          switch cfg.beam_freq  % Get Current and Voltage in Amps and Volts
              case 75
                 ens.adc(1:2,k)=ens.adc(1:2,k).*[43838 ;2092719]/1000000;
              case {150,300}
                 ens.adc(1:2,k)=ens.adc(1:2,k).*[11451 ; 592157]/1000000;
              case 600
                 ens.adc(1:2,k)=ens.adc(1:2,k).*[11451 ; 380667]/1000000;
              case {1200,2400}
                 ens.adc(1:2,k)=ens.adc(1:2,k).*[11451 ; 253765]/1000000;
          end
          %  fprintf('- %d - %f  %f \n',cfg.beam_freq,ens.adc(1:2,k));
      elseif strcmp(cfg.name,'river')
          ens.adc(2,k)=ens.adc(2,k)*.1;
      end
      nbyte=2+40;
      ...
      elseif strcmp(cfg.name,'pioneer-dvl') % Pioneer DVL
          ...
          % Additional bytes we aren't decoding here; health status, leak
          % counts, tx voltage & current, tranducer impedance
          fseek(fd,11,'cof');
          nbyte=nbyte+11;
    ...
    case {'0300'}  % Echo Intensities  (RSSI)
    ...
    case {'0600'} % Bottom track
      ...
      if fix(cfg.prog_ver)==56  % RiverRay
        %% AA=char(fread(fd,87,'char')')
        %  fseek(fd,22,'cof'); % Skip over a bunch of stuff
          fseek(fd,14,'cof'); % Skip over a bunch of stuff
          ens.bt_range(1:4,k)=fread(fd,4,'uint16')*.01; % Surprise to find this here (undocumented)
          ii=ens.bt_range(:,k)==0;                     
          if any(ii), ens.bt_range(ii,k)=NaN; end
          ens.bt_vel(:,k)  =fread(fd,4,'int16')*.001; %?
     %fprintf('%d %d %d %d\n',ens.bt_vel(:,k));
          ii=ens.bt_vel(:,k)==-32768*.001;                  
          if any(ii),ens.bt_vel(ii,k)=NaN;  end
       
          ens.bt_corr(:,k)=fread(fd,4,'uint8');      %  
          ens.bt_ampl(:,k)=fread(fd,4,'uint8');      %  
          ens.bt_perc_good(:,k)=fread(fd,4,'uint8'); %  
    %      fseek(fd,86-45,'cof'); % Skip over a bunch of stuff
          fseek(fd,78-45,'cof'); % Skip over a bunch of stuff
          ens.bt_range(1:4,k)=ens.bt_range(1:4,k)+fread(fd,4,'uint8')*655.36; % undocumented!
          fseek(fd,86-82,'cof'); % Skip over a bunch of stuff
    % fprintf('%f %f %f %f\n',ens.bt_range(:,k));
    % ens.bt_range(:,k)=fread(fd,4,'uint8')*.01; % Least signicant byte of range
          dummy=fread(fd,4,'uint8')*.01; % Least signicant byte of range
    % printf('%f %f %f %f\n',ens.bt_range(:,k));
          nbyte=2+87;
    ...
    %%           fprintf('qual==%d,%f %f',qual,ens.slatitude(k),ens.slongitude(k));
         ...
         if cfg.prog_ver == 57.26  
             if fix(SOURCE)~=1  % All but VmDas.
              % Skip reading BT range MSBs
                fseek(fd,6,'cof');
                ens.bt_range(:,k)=ens.bt_range(:,k)+fread(fd,4,'uint8')*655.36;
             else
                % The last two bytes, beam 3 & 4 range MSB, are corrupted - at
                % least when bottom track data is the last message in the ensemble
                % and data are acquired by VmDAS.  This applies to all files,
                % including ENR. - M. Otero Nov/2023
                fseek(fd,7,'cof');
                ens.bt_range(1:2,k)=ens.bt_range(:,k)+fread(fd,2,'uint8')*655.36;
             end
    
              fseek(fd,9,'cof');
              nbyte=nbyte+9; % short one byte in this section for some reason
    
         elseif cfg.prog_ver>=5.3    % Version 4.05 firmware seems to be missing these last 11 bytes.
              fseek(fd,78-71,'cof');  
              ens.bt_range(:,k)=ens.bt_range(:,k)+fread(fd,4,'uint8')*655.36;
              nbyte=nbyte+11;
...
% block. 
    ...
    case {'2022'}  % New NMEA data block from WInRiverII
      ...
      if SOURCE~=3, fprintf('\n***** Apparently a WINRIVER II file  - GGA/HDT handler only implemented \n\n'); end
      SOURCE=3;
     ...
     if DEBUGFLAG, fprintf('   Specific ID= %d  \n',specID); end

      switch specID
          case 4
              if DEBUGFLAG
                  fprintf('Internal GGA handler not implemented\n');
                  str=char(fread(fd,msgsiz,'uchar')');
                  fprintf('%s\n',str);
                  nbyte=nbyte+msgsiz;
                  msgsiz=0;  
              end  
         ...
         case 5
               if DEBUGFLAG, fprintf('Internal VTG handler not implemented\n');end 

         case 100    % Winriver II v<2.000 GGA
          ...
          case 101     % WinriverII v<2.000 VTG not implemented
             if DEBUGFLAG, fprintf('VTG handler not implemented\n');end           

          case 102     % WinriverII v<2.000 DBT not implemented
              if DEBUGFLAG, fprintf('DBT handler not implemented\n');end           

          case 103     % WinriverII v<2.000 HDT not implemented
               if DEBUGFLAG, fprintf('HDT handler not implemented\n');end           
                    
          case 104   % WinriverII v>=2.000 GGA
            %%fprintf('4');
          gpstring=char(fread(fd,7,'uchar')');
          utc=char(fread(fd,10,'uchar')');
          try ens.smtime(k)=(sscanf(utc(1:2),'%d')+sscanf(utc(3:4),'%d')/60+sscanf(utc(5:6),'%d')/3600)/24; end  % Added Aug/23
          ens.slatitude(k)=fread(fd,1,'double');
          if char(fread(fd,1,'uchar'))=='S'
             ens.slatitude(k)=-ens.slatitude(k);
          end   
          ...
          nbyte=nbyte+35;
          msgsiz=msgsiz-35;
        case 105     % WinriverII v>=2.000 VTG not implemented
               if DEBUGFLAG, fprintf('VTG handler not implemented\n');end           
                    
        case 106     % WinriverII v>=2.000 DBT not implemented
               if DEBUGFLAG, fprintf('DBT handler not implemented\n');end           
                     
        case 107     % WinriverII v>=2.000 HDT  
              headstring=char(fread(fd,7,'uchar')');
              heading=fread(fd,1,'double');    
              if heading~=0, ens.heading(k)=heading; end  % Overwrite internal values if external data available.
              nbyte=nbyte+15;
              msgsiz=msgsiz-15;
          case 204     % NMEA GGA string 
              str=char(fread(fd,msgsiz,'uchar')');
          %    fprintf('%s\n',str);
              ii=find(str==',');
              utc=str(ii(1)+1:ii(2)-1);
              try ens.smtime(k)=(sscanf(utc(1:2),'%d')+sscanf(utc(3:4),'%d')/60+sscanf(utc(5:end),'%f')/3600)/24; end  %   
              ens.slatitude(k)=sscanf(str(ii(2)+[1:2]),'%d')+sscanf(str(ii(2)+3:ii(3)-1),'%f')/60;
              if str(ii(3)+1)=='S'
                 ens.slatitude(k)=-ens.slatitude(k);
              end   
              ens.slongitude(k)=sscanf(str(ii(4)+[1:3]),'%d')+sscanf(str(ii(4)+4:ii(5)-1),'%f')/60;
              if str(ii(5)+1)=='W'
                  ens.slongitude(k)=-ens.slongitude(k);
              end   
              nbyte=nbyte+msgsiz;
              msgsiz=0;
          case 205     % NMEA VTG string
              if DEBUGFLAG
                  fprintf('VTG handler not implemented\n');          
                  str=char(fread(fd,msgsiz,'uchar')');
                  fprintf('%s\n',str);
                  nbyte=nbyte+msgsiz;
                  msgsiz=0;
       ...
       case 206     % NMEA DBT
          ...
          case 207     % NMEA HDT
              if DEBUGFLAG, fprintf('HDT handler not implemented\n');end           
          case 200     % NMEA other
              if DEBUGFLAG, fprintf('NMEA OTHER handler not implemented\n');end           
          otherwise
             if DEBUGFLAG, fprintf('Other handlers not implemented\n'); end 
      end
      fseek(fd,msgsiz,'cof');
      nbyte=nbyte+msgsiz;
...
% Can't remember what these are for: experimental 5-beam system?      
    ...
    case '4100'   % RiverRay Vertical Beam Range
      
      fseek(fd,2,'cof');
      rangetobottom=fread(fd,1,'int32')*.001; % meters
      ens.bt_range(5,k)=rangetobottom;
      fseek(fd,1,'cof');
      nbyte=2+7;

    case '4400'   % RiverRay "Firmware Status"
      fseek(fd,28,'cof');
      nbyte=2+28;
...
% If the blocks sometimes have missing GPS blocks,
% the initialization sometime fails - this will
% return the right source if ANY of the blocks is
% in the initialization has a GPS block.
switch SOURCE
    case 0
...
function alpha=absorb(f,S,T,p)
% ABSORB Absorption of sound
%  A=ABSORB(F,S,T,P) is the absorption coefficient (dB/km) at a frequency of
%  F kHz, PSS78 salinity S, temperature T (deg C), and sea pressure p (dbar)
% 
% As per Francois and Garrison, "Sound absorption based on ocean 
% measurements: Part II: Boric acid contribution and equation for total 
% absorption," Journal of the Acoustical Society of America, vol. 72, 
% no. 6, pp. 1879-1890, 1982

pH=8;

%c=1492.9+3*(T-10)-0.006*(T-10)^2-0.04*(T-18).^2 ...
%  +1.2*(S-35)-0.01*(T-18).*(S-35)+p/61;

c=1412+3.21*T+1.19*S+.0167*p;
  
A1=(8.86./c)*10.^(0.78*pH-5);
f1=2.8*sqrt(S/35).*10.^(4-1245./(T+273));
P1=1;
A2=21.44*(S./c).*(1+.025*T);
P2=1-1.37e-4*p+6.2e-9*p.*p;
f2=8.17*10.^(8-1990./(T+273))./(1+.0018*(S-35));
P3=1-3.83e-5*p+4.9e-10*p.*p;
